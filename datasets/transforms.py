...
# Random resize image size with rito (1-max_stretch, 1+max_stretch)
class RandomStretch(object):
    def __init__(self, max_stretch=0.05, interpolation='bilinear'):
        self.max_stretch = max_stretch
        self.interpolation = interpolation

    def __call__(self, img): #使实例能够像函数一样被调用
        scale = 1.0 + tf.random_uniform([], -self.max_stretch, self.max_stretch)#1±0.05
        img_shape = tf.shape(img)
        ts = tf.to_int32(tf.round(tf.to_float(img_shape[:2]) * scale))
        resize_method_map = {'bilinear': tf.image.ResizeMethod.BILINEAR, 'bicubic': tf.image.ResizeMethod.BICUBIC}
